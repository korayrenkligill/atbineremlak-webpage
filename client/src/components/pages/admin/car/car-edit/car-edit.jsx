...
function CarEdit({ user }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!error) {
      ...
      try {
        await axios
          .put(`${BACKEND_URL}/cars/${id}`, formData, {
            headers: {
  ...
  useEffect(() => {
    axios.get(`${BACKEND_URL}/cars/${id}`).then((response) => {
      ...
      setPazarlik(response.data.pazarlik);
      setMarka(response.data.marka);
      setSeri(response.data.seri);
      setModel(response.data.model);
      setYil(response.data.yil);
      setYakit(response.data.yil);
      setVites(response.data.vites);
      setAracDurumu(response.data.aracDurumu);
      setKm(response.data.km);
      setKasa(response.data.kasa);
      setMotorGucu(response.data.motorGucu);
      setMotorHacmi(response.data.motorHacmi);
      setCekis(response.data.cekis);
      setRenk(response.data.renk);
      setAgirHasarli(response.data.agirHasarli);
      setTakas(response.data.takas);
      ...
      setGuvenlik(response.data.guvenlik);
      setIcDonanim(response.data.icDonanim);
      setDisDonanim(response.data.disDonanim);
      setMultimedya(response.data.multimedya);
  ...
  return (
    <div className="car-add">
      <h1 className="admin-title">Otomobil İlanı Düzenle</h1>
